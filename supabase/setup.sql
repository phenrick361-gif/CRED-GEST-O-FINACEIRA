...
-- =========================================================
-- MIGRATION: Contratos Parcelados
-- Execute este bloco no SQL Editor para adicionar a funcionalidade de contratos parcelados
-- =========================================================

\begin

-- Adicionar tipo de contrato na tabela emprestimos
alter table public.emprestimos
...
create index if not exists installments_due_date_idx on public.installments(contract_id, due_date);
create index if not exists installments_status_idx on public.installments(contract_id, status);

-- RLS
alter table public.installments enable row level security;

drop policy if exists "installments_select_own" on public.installments;
create policy "installments_select_own" on public.installments for select to authenticated using (auth.uid() = user_id);

drop policy if exists "installments_insert_own" on public.installments;
create policy "installments_insert_own" on public.installments for insert to authenticated with check (auth.uid() = user_id and exists (select 1 from public.emprestimos e where e.id = contract_id and e.user_id = auth.uid()));

drop policy if exists "installments_update_own" on public.installments;
create policy "installments_update_own" on public.installments for update to authenticated using (auth.uid() = user_id) with check (auth.uid() = user_id);

drop policy if exists "installments_delete_own" on public.installments;
create policy "installments_delete_own" on public.installments for delete to authenticated using (auth.uid() = user_id);

-- Função para contagem de atrasados
create or replace function public.get_installment_overdue_count()
returns bigint language sql security definer as $$
  select count(*) from public.installments where user_id = auth.uid() and paid_at is null and due_date < current_date;
$$;
